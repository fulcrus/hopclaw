...
func (c *Capability) prepareRoutedRequest(req captypes.InvokeRequest) (captypes.InvokeRequest, capprofile.ExecutionTrace) {
	...
	target := capprofile.TargetContext{
		Surface:        capprofile.SurfaceDesktop,
		Operation:      strings.TrimSpace(req.Operation),
		SemanticAction: inferDesktopSemanticAction(req.Operation, params),
		AppName:        normalize.FirstNonEmpty(normalize.String(params["app"]), state.AppName),
		BundleID:       normalize.FirstNonEmpty(normalize.String(params["bundle_id"]), state.BundleID),
		DriverID:       normalize.FirstNonEmpty(normalize.String(params["driver_id"]), state.DriverID),
		Title:          normalize.FirstNonEmpty(normalize.String(params["title_contains"]), state.Title),
	}
	...
	applyDesktopRouteHints(params, target, trace)
...
func (c *Capability) decorateInvokeResult(operation string, result *captypes.InvokeResult, trace capprofile.ExecutionTrace) {
	if result == nil {
		return
	}
	...
	if result.Data == nil {
		result.Data = make(map[string]any, 6)
	}
	if trace.ProfileID != "" {
	...
	evidence["routing"] = trace.MetadataMap()
...
func (c *Capability) rememberSessionState(sessionID string, req captypes.InvokeRequest, result *captypes.InvokeResult, trace capprofile.ExecutionTrace) {
	...
	state.Surface = capprofile.SurfaceDesktop
	state.AppName = normalize.FirstNonEmpty(desktopString(result.Data["app"]), normalize.String(req.Params["app"]), state.AppName)
	state.BundleID = normalize.FirstNonEmpty(desktopString(result.Data["bundle_id"]), normalize.String(req.Params["bundle_id"]), state.BundleID)
	state.DriverID = normalize.FirstNonEmpty(normalize.String(result.Data["driver_id"]), normalize.String(req.Params["driver_id"]), state.DriverID)
	state.Title = normalize.FirstNonEmpty(normalize.String(result.Data["title"]), normalize.String(req.Params["title_contains"]), state.Title)
...
func inferDesktopSemanticAction(operation string, params map[string]any) string {
	...
	case desktoptypes.ActionOpenApp:
		return "app.launch"
	case desktoptypes.ActionFocusApp, desktoptypes.ActionFocusWindow:
		return "app.focus"
	case desktoptypes.ActionInvokeCommand:
		return "command.invoke"
	case desktoptypes.ActionInvokeDriverAction:
		return strings.TrimSpace(normalize.String(params["semantic_action"]))
	case desktoptypes.ActionTypeText:
		return "app.type"
	case desktoptypes.ActionHotkey:
		return "app.hotkey"
	case desktoptypes.ActionFindElement, desktoptypes.ActionGetElementValue, desktoptypes.ActionAssertElement, desktoptypes.ActionCaptureTree, desktoptypes.ActionListApps, desktoptypes.ActionListWindows, desktoptypes.ActionListCommands, desktoptypes.ActionListDriverActions, desktoptypes.ActionScreenshot, desktoptypes.ActionFindText:
		return "app.inspect"
	case desktoptypes.ActionClickElement, desktoptypes.ActionClickText, desktoptypes.ActionSetElementValue, desktoptypes.ActionClearElement:
		return "app.interact"
	default:
...
func inferDesktopTransport(operation string, data map[string]any) string {
	if transport := strings.TrimSpace(normalize.String(data["transport"])); transport != "" {
		return transport
	}
	switch strings.TrimSpace(operation) {
	case desktoptypes.ActionOpenApp, desktoptypes.ActionFocusApp, desktoptypes.ActionFocusWindow:
		return capprofile.TransportNativeCommand
	case desktoptypes.ActionInvokeCommand:
		return capprofile.TransportMenuInvoke
	case desktoptypes.ActionInvokeDriverAction, desktoptypes.ActionFindElement, desktoptypes.ActionClickElement, desktoptypes.ActionSetElementValue, desktoptypes.ActionClearElement, desktoptypes.ActionGetElementValue, desktoptypes.ActionAssertElement, desktoptypes.ActionCaptureTree:
		return capprofile.TransportSemanticUIAction
	case desktoptypes.ActionTypeText:
		return capprofile.TransportStructuredText
	case desktoptypes.ActionHotkey:
		return capprofile.TransportVerifiedHotkey
	case desktoptypes.ActionFindText, desktoptypes.ActionClickText, desktoptypes.ActionScreenshot:
		return capprofile.TransportOCRAnchoredVisual
	default:
		return capprofile.TransportSemanticUIAction
	}
}

func applyDesktopRouteHints(params map[string]any, target capprofile.TargetContext, trace capprofile.ExecutionTrace) {
	if _, ok := params["target_scope"]; !ok {
		scope := map[string]any{
			"surface": "desktop",
		}
		if target.AppName != "" {
			scope["app"] = target.AppName
		}
		if target.BundleID != "" {
			scope["bundle_id"] = target.BundleID
		}
		if target.DriverID != "" {
			scope["driver_id"] = target.DriverID
		}
...
func desktopString(value any) string {
	...
	case map[string]any:
		return normalize.FirstNonEmpty(normalize.String(typed["name"]), normalize.String(typed["title"]))
	default:
